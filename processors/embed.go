...
// script file.
//
//go:embed all:git-filter all:npm-filter all:cargo-filter all:compress all:casefolder all:shorthand all:stacktrace-filter all:pytest-filter all:ndjson-filter all:pdf-to-md all:cordon-filter all:lint-filter all:trivy-filter all:go-test-filter all:har-filter all:mhtml-filter
var FS embed.FS
