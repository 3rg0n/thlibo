...
fixes are cases where a safety net looked present and was not: a PDF that
killed the process outright instead of failing open (a `runtime.throw`, which
`recover()` cannot catch), a CVE gate in CI that had been passing without
scanning anything for over a week, and a documented 30-minute fuzz soak whose
job budget guaranteed cancellation before it could report. The installer's WSL
advisory — dead since ADR 0005 removed the APE binary it protected — is
replaced by one for a failure that does exist. No behaviour change on the
compression path.
