...
| B5 | Routing call sent to daemon when no fast-path match. Prompt is built from registry descriptions; first 200 chars of input included. | §Routing | UT + IT | UT: routing prompt builder output matches fixture. IT: live daemon returns a processor name; middleware dispatches to it. |
