...
| D2 | ✅ Codex adapter uses PostToolUse `decision:block` + `reason` to replace tool output with a compressed version. | §Client adapters | UT + Manual | UT: 11 tests (7 hooks.json + 4 TOML-feature-flag merger) exercise every state. Hook script validated against the canonical envelope shape from developers.openai.com/codex/hooks. `thlibo install --codex` writes the hook, merges `~/.codex/hooks.json` (preserving other hooks), and sets `[features] codex_hooks = true` in `config.toml`. Manual E2E on a machine with Codex installed records numbers in release notes. |
...
| # | Requirement | Spec | Verification | Pass condition |
|---|---|---|---|---|
| ~~E1~~ | ~~`thlibo install` creates `thlibo-users` group, adds current user.~~ | ~~§Install flow~~ | **Not needed in v0.1.** | Superseded by the per-user autostart model: the daemon runs as the current user, no shared group required. Windows pipe ACL already grants current-user-only. The row is kept here struck-through so future contributors see it was considered and deliberately dropped. |
| E2 | ✅ `thlibo install` registers the daemon for auto-launch at user logon. | §Install flow | UT + Manual | Per-user autostart via Windows Startup folder / macOS LaunchAgent / Linux systemd user unit. Each backend implements the same `install.Installer` interface (Install/Uninstall/Status/Mechanism). Verified live on Windows: generated `.cmd` in `%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup\` launches `thlibod`, which binds named pipes and reports ready. |
| E3 | ✅ `thlibo install` populates `~/.thlibo/processors/` with the embedded built-ins so they're editable on disk. | §Install flow, §Directory layout | UT + Manual | `install.MirrorBuiltins` copies the go:embed FS to disk with correct modes (run.py → 0o700, descriptors → 0o600). Idempotent re-run. Verified in the live sandbox run. |
| E4 | ✅ `thlibo install` merges the PreToolUse hook into `~/.claude/settings.json` without clobbering existing hooks. | §Install flow | UT + Manual | 7 unit tests cover fresh-file, preserve-unrelated-keys, 3-pass idempotency, path-change recognition, backslash-path upgrade, refuse-invalid-JSON. Manual install on a machine with an existing settings.json (git-ai hooks, model selection, etc.) — all other keys preserved verbatim. |
| E5 | ✅ `thlibo pull gemma-4-e4b` downloads the Q4_K_M GGUF to `~/.thlibo/models/` with a progress indicator and verifies integrity. | §Install flow | UT + Manual | `install.Pull` + `cmd/thlibo/pullcmd`: HTTPS (loopback HTTP permitted for testing), HTTP Range resume, SHA-256 verification, idempotent second run, progress callback, context cancellation. 10 unit tests against httptest.Server cover every path without touching the real network. Pinning the canonical SHA of the released GGUF is a one-line edit (`DefaultModel.ExpectedSHA256` in `internal/install/model.go`). |
...
| F6 | ✅ Token savings measured and recorded. | §Token savings estimate | UT + Manual | `internal/middleware.TestTokenSavingsTable` runs each script processor against a representative fixture and logs a markdown table of raw/compressed/reduction. Results from 2026-05-13: git-filter (diff) 97.6%, npm-filter 99.4%, cargo-filter 89.2%, git-filter (status) 5.4%. Published in [.plan/release-notes-0.1.0.md](release-notes-0.1.0.md). Prompt processors (compress/casefolder) measured against a live daemon at release-machine smoke test. |
