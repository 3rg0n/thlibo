...
// readKeyword reads a bare token: an operator, `true`/`false`/`null`, or
// anything else that isn't a string, name, number, or bracket.
//
// thlibo: the stray-delimiter guard below is a local fix. NextToken
// dispatches `)`, `{`, and `}` here — none of them has a case of its own —
// and the scan loop stops on a delimiter immediately, so upstream returned
// a zero-length keyword *without advancing pos*. Every caller loops on
// NextToken until EOF or error, so one stray `)` in a content stream was an
// infinite loop: page text extraction never returned.
//
// That is worse than a crash under thlibo's fail-open contract. A panic is
// recovered and the original bytes pass through; a hang has nothing to
// recover — the hook blocks and the AI client waits on it. Found by
// FuzzParseInlineDict on the one-byte input ")".
func (l *Lexer) readKeyword() (Token, error) {
	...
	if l.pos == start {
		// A delimiter that reached this far is malformed content: `)` with no
		// open paren, or a `{`/`}` from a PostScript function that leaked into
		// an object stream. Consume it and report it as an empty keyword so
		// callers resynchronise on the next real token. Progress is the
		// invariant that matters — every NextToken caller loops until EOF.
		l.pos++
		return Token{Type: TKeyword, Str: ""}, nil
	}
	s := string(l.data[start:l.pos])
	switch s {
	case "true":
