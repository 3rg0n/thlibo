...
func extractTextWithResources(content []byte, fonts map[Name]Dict, reader *Reader, resources Dict, depth int) []TextSpan {
	...
	currentMCID := func() int {
		for i := len(markedStack) - 1; i >= 0; i-- {
			if markedStack[i].mcid >= 0 {
				return markedStack[i].mcid
			}
		}
		return -1
	}
	...
	for {
		...
		case "BMC":
			...
			markedStack = append(markedStack, markedEntry{mcid: -1})
